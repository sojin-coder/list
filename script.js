...
//   });
 import { initializeApp } from "https://www.gstatic.com/firebasejs/12.6.0/firebase-app.js";
  ...
  // 🔥 ប្រើ query ជាមួយ orderBy តាម createdAt (ពីមុនមកក្រោយ)
  const colRef = collection(db, "students");
  ...
  const khmerNumbers = { '០':'0','១':'1','២':'2','៣':'3','៤':'4','៥':'5','៦':'6','៧':'7','៨':'8','៩':'9' };

  function convertKhmerToLatin(str) {
    if (!str) return '';
    let converted = '';
    for (let char of str) converted += khmerNumbers[char] || char;
    return converted;
  }

  function extractNumberFromInput(inputStr) {
    let latinStr = convertKhmerToLatin(inputStr);
    const match = latinStr.match(/[\d.]+/);
    return match ? parseFloat(match[0]) : NaN;
  }

  function detectCurrency(amount) {
    return amount >= 1000 ? "KHR" : "USD";
  }
  ...
  function formatMoney(amount, currency) {
    ...
    if (currency === "USD") {
      ...
      return new Intl.NumberFormat('km-KH', { style: 'currency', currency: 'KHR', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(num);
    }
  }

  function getCurrencyBadge(currency) {
    if (currency === "USD") return '<span class="currency-badge usd"><i class="fas fa-dollar-sign"></i> USD</span>';
    return '<span class="currency-badge khr"><i class="fas fa-riel-sign"></i> KHR</span>';
  }
  ...
  function updateCurrencyUI(amount) {
    const usdOpt = document.getElementById('usdOption');
    const khrOpt = document.getElementById('khrOption');
    const num = parseFloat(amount);
    if (!isNaN(num) && num > 0) {
      if (detectCurrency(num) === "USD") {
        usdOpt.classList.add('selected'); khrOpt.classList.remove('selected');
      } else {
        khrOpt.classList.add('selected'); usdOpt.classList.remove('selected');
      }
    } else {
      usdOpt.classList.remove('selected'); khrOpt.classList.remove('selected');
    }
  }
  ...
  function updateTotalSummary(dataArray) {
    ...
    dataArray.forEach(item => {
      if (item.currency === "USD") totalUSD += item.money;
      else totalKHR += item.money;
    });
    const usdF = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 });
    const khrF = new Intl.NumberFormat('km-KH', { style: 'currency', currency: 'KHR', minimumFractionDigits: 0 });
    document.getElementById('totalUSD').innerHTML = usdF.format(totalUSD);
    document.getElementById('totalKHR').innerHTML = khrF.format(totalKHR);
    document.getElementById('grandTotalUSD').innerHTML = usdF.format(totalUSD + (totalKHR / EXCHANGE_RATE));
  }

  // 🔥 REAL-TIME LISTENER ជាមួយ ORDER BY (បង្ហាញតាមលំដាប់បញ្ចូល)
  let allData = [];
  onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      const data = docSnap.data();
      const moneyVal = data.money || 0;
      const currency = data.currency || detectCurrency(moneyVal);
      
      allData.push({ id: docSnap.id, money: moneyVal, currency: currency });
      
      rows += `
        <tr>
          <td class="text-center">${index++}</td>
          <td class="fw-semibold"><i class="fas fa-id-card me-2"></i>${escapeHtml(data.full_name || "—")}</td>
          <td><i class="fas fa-location-dot me-1"></i> <small>${escapeHtml(data.address || "—")}</small></td>
          <td class="text-end fw-bold ${currency === 'USD' ? 'amount-usd' : 'amount-khr'}">
            ${formatMoney(moneyVal, currency)}${getCurrencyBadge(currency)}
          </td>
          <td class="text-center">
            <button class="icon-btn edit" onclick="window.editEntry('${docSnap.id}', '${escapeHtml(data.full_name || "")}', '${escapeHtml(data.address || "")}', ${moneyVal}, '${currency}')">
              <i class="fas fa-pen"></i>
            </button>
    ...
    if (!rows) rows = `<tr><td colspan="5" class="text-center py-5"><i class="fas fa-inbox fa-2x"></i><br>មិនទាន់មានទិន្នន័យ</td></tr>`;
    
    document.getElementById("listBody").innerHTML = rows;
    document.getElementById("countLabel").innerHTML = `<i class="fas fa-user-check me-1"></i>${snapshot.size} នាក់`;
    updateTotalSummary(allData);
  });

  // Preview converted number
  const moneyInput = document.getElementById("moneyInp");
  const convertedSpan = document.getElementById("convertedNumber");
  moneyInput.addEventListener("input", (e) => {
    const num = extractNumberFromInput(e.target.value);
    if (!isNaN(num)) {
      convertedSpan.innerHTML = num.toString();
      updateCurrencyUI(num);
    } else {
      convertedSpan.innerHTML = "-";
      updateCurrencyUI(NaN);
    }
  });
  ...
  addForm.addEventListener("submit", async (e) => {
    ...
    const nameVal = document.getElementById("nameInp").value.trim();
    const addrVal = document.getElementById("addrInp").value.trim();
    let moneyVal = extractNumberFromInput(document.getElementById("moneyInp").value);
    if (isNaN(moneyVal)) moneyVal = 0;
    
    if (!nameVal) return alert("សូមបញ្ចូលឈ្មោះពេញ");
    if (moneyVal <= 0) return alert("សូមបញ្ចូលចំនួនទឹកប្រាក់ត្រឹមត្រូវ");
    ...
    try {
      await addDoc(colRef, {
        ...
        address: addrVal || "មិនបានបញ្ជាក់",
        money: moneyVal,
        currency: detectCurrency(moneyVal),
        createdAt: new Date().toISOString() // 🔥 បន្ថែមពេលវេលាសម្រាប់តម្រៀប
      });
      addForm.reset();
      convertedSpan.innerHTML = "-";
      document.getElementById('usdOption').classList.remove('selected');
      ...
      btn.innerHTML = '<i class="fas fa-check-circle"></i> រក្សាទុកដោយជោគជ័យ!';
      btn.style.background = "linear-gradient(95deg, #00c6ff, #0aff9d)";
      setTimeout(() => { btn.innerHTML = original; btn.style.background = "linear-gradient(95deg, #6d481d, #753901)"; }, 2000);
    } catch (err) {
      alert("មានបញ្ហាក្នុងការរក្សាទុកទិន្នន័យ");
    }
  });
  ...
  window.deleteEntry = async (id) => {
    if (confirm("តើអ្នកពិតជាចង់លុបមែនទេ?")) {
      ...
      catch (error) { alert("លុបមិនបាន: " + error.message); }
    }
  };
  ...
  window.editEntry = (id, name, address, money, currency) => {
    ...
    document.getElementById("editAddr").value = address;
    document.getElementById("editMoney").value = money;
    document.getElementById("editCurrencySelect").value = currency;
    modalEdit.show();
  };
  ...
  document.getElementById("updateBtn").addEventListener("click", async () => {
    ...
    const newCurrency = document.getElementById("editCurrencySelect").value;
    if (isNaN(newMoney)) newMoney = 0;
    if (!newName) return alert("សូមបញ្ចូលឈ្មោះពេញ");
    try {
      ...
      modalEdit.hide();
    } catch (err) { alert("ការកែប្រែបរាជ័យ: " + err.message); }
  });